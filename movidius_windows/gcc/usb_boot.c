...
//---------------------------------------------------------------------------
void usb_library_load(void)
{
        initialized = !libusb_init(NULL);
}

void usb_library_unload(void)
{
        if (initialized)
                libusb_exit(NULL);
}
...
static const char *gen_addr(libusb_device *dev)
{
        ...
        if(pnum_cnt == LIBUSB_ERROR_OVERFLOW) {
                strcpy(buff, "<error>");               // shouldn't happen!
                return buff;
        }
        p = buff;
...
//---------------------------------------------------------------------------
int usb_find_device(unsigned idx, char *addr, unsigned addr_size, void **device, int vid, int pid)
{
        ...
        if(!initialized) {
                PRINT_INFO("Library has not been initialized when loaded\n");
                return MVNC_ERROR;
        }
        if(!devs || idx == 0) {
                if (devs) {
                ...
                if((res = libusb_get_device_list(NULL, &devs)) < 0) {
                        PRINT_INFO("Unable to get USB device list: %s\n", libusb_strerror(res));
                        return MVNC_ERROR;
                }
        }
        ...
        while((dev = devs[i++]) != NULL) {
                if((res = libusb_get_device_descriptor(dev, &desc)) < 0) {
                        PRINT_INFO("Unable to get USB device descriptor: %s\n", libusb_strerror(res));
                        continue;
                }
//      PRINT_DEBUG("Scan VID/PID %04x:%04x\n", desc.idVendor, desc.idProduct);
                if((desc.idVendor == vid && desc.idProduct == pid) || (pid == 0 && vid == 0 && ((desc.idVendor == DEFAULT_VID && desc.idProduct == DEFAULT_PID) || (desc.idVendor == DEFAULT_OPEN_VID && desc.idProduct == DEFAULT_OPEN_PID)))) {
                        if(device) {
                                const char *caddr = gen_addr(dev);
                                if(!strcmp(caddr, addr)) {
                                        PRINT_DEBUG("Found Address: %s - VID/PID %04x:%04x\n", addr, desc.idVendor, desc.idProduct);
                                ...
                                const char *caddr = gen_addr(dev);
                                PRINT_DEBUG("Found Device %d Address: %s - VID/PID %04x:%04x\n", idx, caddr, desc.idVendor, desc.idProduct);
...
//---------------------------------------------------------------------------
static libusb_device_handle *usb_open_device(libusb_device *dev, uint8_t *endpoint, char *err_string_buff, unsigned buff_size)
{
        ...
        if((res = libusb_set_configuration(h, 1)) < 0) {
                snprintf(err_string_buff, buff_size, "setting config 1 failed: %s\n", libusb_strerror(res));
                libusb_close(h);
                return 0;
        }
        if((res = libusb_claim_interface(h, 0)) < 0) {
                snprintf(err_string_buff, buff_size, "claiming interface 0 failed: %s\n", libusb_strerror(res));
                libusb_close(h);
                return 0;
        }
        if((res = libusb_get_config_descriptor(dev, 0, &cdesc)) < 0) {
                snprintf(err_string_buff, buff_size, "Unable to get USB config descriptor: %s\n", libusb_strerror(res));
                libusb_close(h);
                return 0;
        }
...
static int wait_findopen(const char *device_address, int timeout, libusb_device ** dev, libusb_device_handle ** devh, uint8_t * endpoint)
{
        ...
        if(mvnc_loglevel > 1) {  // I know about switch(), but for some reason -1 is picked up correctly
                if(timeout == -1)
        ...
        i = 0;
        for(;;) {
                rc = usb_find_device(0, (char *) device_address, 0, (void **) dev, DEFAULT_VID, DEFAULT_PID);
                if(rc < 0)
                        return MVNC_ERROR;
                if(!rc) {
                        if((*devh = usb_open_device(*dev, endpoint, last_open_dev_err, 128))) {
                                PRINT_DEBUG("Found and opened device\n");
                                return 0;
                        }
...
//---------------------------------------------------------------------------
static int send_file(libusb_device_handle * h, uint8_t endpoint, const uint8_t * tx_buf, unsigned file_size)
{
        const uint8_t *p;
        int rc;
        int wb, twb, wbr;
...
//      highres_time_t t1, t2;
    clock_t t1, t2;
        ...
        twb = 0;
        p = tx_buf;
        PRINT_DEBUG("Performing bulk write of %u bytes...\n", file_size);

        while(twb < file_size) {
//              highres_gettime(&t1);
        t1 = clock();
                wb = file_size - twb;
                if(wb > bulk_chunk_len) wb = bulk_chunk_len;
                wbr = 0;
                rc = libusb_bulk_transfer(h, endpoint, (void *) p, wb, &wbr, write_timeout);

                if(rc || (wb != wbr)) {
                        if(rc == LIBUSB_ERROR_NO_DEVICE) break;
                        PRINT_INFO("bulk write: %s (%d bytes written, %d bytes to write)\n", libusb_strerror(rc), wbr, wb);
                        if(rc == LIBUSB_ERROR_TIMEOUT) return MVNC_TIMEOUT;
                        else                           return MVNC_ERROR;
                }
//              highres_gettime(&t2);
        t2 = clock();
                elapsed_time += (double)((t2 - t1)/CLK_TCK);
                twb += wbr;
                p += wbr;
        }
        PRINT_DEBUG("Successfully sent %u bytes of data in %lf ms (%lf MB/s)\n", file_size, elapsed_time, ((double) file_size / 1048576.) / (elapsed_time * 0.001));
        return 0;
}
